...
class Main:
    ...
    def __init__(self):
        ...
        if self.TYPE == "shortcuts":
            ...
            if self.GROUPING is not None:
                selectedShortcut = LIBRARY.selectShortcut(
                    "", grouping=self.GROUPING, custom=self.CUSTOM, showNone=self.NONE)
            else:
                selectedShortcut = LIBRARY.selectShortcut(
            ...
            if selectedShortcut is not None and selectedShortcut.getProperty(
                ...
                if self.LABEL is not None and selectedShortcut.getLabel() != "":
                    xbmc.executebuiltin(
                        "Skin.SetString(" + self.LABEL + "," + selectedShortcut.getLabel() + ")")
                if self.ACTION is not None:
                    xbmc.executebuiltin(
                        "Skin.SetString(" + self.ACTION + "," + path + " )")
                ...
                if self.LIST is not None:
                    xbmc.executebuiltin(
                        "Skin.SetString(" + self.LIST + "," + DATA.getListProperty(path) + ")")
        ...
        if self.TYPE == "widgets":
            ...
            # Check if we should show the custom option (if the relevant
            # widgetPath skin string is provided and isn't empty)
            showCustom = False
            if self.WIDGETPATH and xbmc.getCondVisibility(
                    "!String.IsEmpty(Skin.String(%s))" % (self.WIDGETPATH)):
                showCustom = True
            ...
            if self.GROUPING:
                if self.GROUPING.lower() == "default":
                    ...
                    selectedShortcut = LIBRARY.selectShortcut(
                        "", grouping=self.GROUPING, custom=showCustom, showNone=self.NONE)
            else:
                selectedShortcut = LIBRARY.selectShortcut(
                    "", grouping="widget", custom=showCustom, showNone=self.NONE)
            ...
            if selectedShortcut is None:
                ...
                xbmc.executebuiltin(
                    "Skin.SetString(%s,%s)" %
                    (self.WIDGETPATH, urllib.unquote(
        ...
        if self.TYPE == "context":
            ...
            if not xbmc.getCondVisibility(
                    "Skin.HasSetting(SkinShortcuts-FullMenu)"):
    ...
    def _manage_shortcuts(self, group, defaultGroup, nolabels, groupname):
        ...
        if homeWindow.getProperty("skinshortcuts-loading") and int(calendar.timegm(
                gmtime())) - int(homeWindow.getProperty("skinshortcuts-loading")) <= 5:
            return
        ...
        # Update home window property (used to automatically refresh
        # type=settings)
        homeWindow.setProperty(
            "skinshortcuts", strftime(
                "%Y%m%d%H%M%S", gmtime()))

        # Clear window properties for this group, and for backgrounds, widgets,
        # properties
                        ...
                        if deleteFile:
                            ...
                            if xbmcvfs.exists(file_path):
                                try:
                                    ...
                                    print_exc()
                                    common.log(
                                        "### ERROR could not delete file %s" % file)
                        else:
    ...
    def _hidesubmenu(self, menuid):
        ...
        while xbmc.getCondVisibility(
            "!String.IsEmpty(Container(%s).ListItem(%i).Property(isSubmenu))" %
                (menuid, count)):
            count -= 1

        if count != 0:
            xbmc.executebuiltin(
                "Control.Move(" + menuid + "," + str(count) + " )")
    ...
    def _resetlist(self, menuid, action):
        count = 0
        while xbmc.getCondVisibility(
            "!String.IsEmpty(Container(%s).ListItemNoWrap(%i).Label)" %
                (menuid, count)):
            count -= 1
