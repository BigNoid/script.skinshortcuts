...
DATAPATH = os.path.join(xbmc.translatePath(
    "special://profile/addon_data/").decode('utf-8'), ADDONID)
SKINPATH = xbmc.translatePath(
    "special://skin/extras/script.skinshortcuts/").decode('utf-8')
DEFAULTPATH = xbmc.translatePath(
    os.path.join(
        CWD,
        'resources',
        'shortcuts').encode("utf-8")).decode("utf-8")
...
class GUI(xbmcgui.WindowXMLDialog):
    ...
    def load_shortcuts(
            self,
            includeUserShortcuts=True,
            addShortcutsToWindow=True):
        ...
        if includeUserShortcuts:
            shortcuts = DATA._get_shortcuts(
                self.group,
                defaultGroup=self.defaultGroup,
                isSubLevel=isSubLevel)
        else:
            shortcuts = DATA._get_shortcuts(
                self.group, defaultGroup=self.defaultGroup, defaultsOnly=True)
    ...
    def _display_listitems(self, focus=None):
        ...
        for listitem in self.allListItems:
            ...
            if listitem.getProperty("visible-condition"):
                shouldDisplay = xbmc.getCondVisibility(
        ...
        if not visible:
            listitem = xbmcgui.ListItem(
                LANGUAGE(32013), iconImage="DefaultShortcut.png")
    ...
    def _parse_shortcut(self, item):
        ...
        # If either localLabel[ 2 ] starts with a $, ask Kodi to parse it for
        # us
    ...
    def _add_additional_properties(self, listitem=None):
        ...
        if customProperties != "":
            ...
            for customProperty in customProperties:
                if customProperty[1].startswith("$") and not customProperty[
                        1].startswith("$SKIN"):
                    # Translate some listItem properties if needed so they're
                    # displayed correctly in the gui
                    allProps[
                        customProperty[0]] = xbmc.getInfoLabel(
                        customProperty[1])
                else:
                    allProps[
                        customProperty[0]] = DATA.local(
                        customProperty[1])[2]
                    if customProperty[1].isdigit():
                        allProps["%s-NUM" %
                                 (customProperty[0])] = customProperty[1]

                # if this is backgroundName or backgroundPlaylistName, keep
                # them so we can localise them properly
        ...
        # If we've kept backgroundName, localise it with the updated playlist
        # name
        if backgroundName is not None and backgroundPlaylistName is not None:
            allProps["backgroundName"] = DATA.local(backgroundName)[2].replace(
                "::PLAYLIST::", backgroundPlaylistName)

        # Get fallback properties
        fallbackProperties, fallbacks = DATA._getCustomPropertyFallbacks(
            self.group)
        ...
        for key in otherProperties:
            if key in allProps.keys() and key in requires.keys() and requires[
                    key] not in allProps.keys():
        ...
        added, removed, changed = self.DictDiffer(
            allProps, self.windowProperties)
        for key in added:
    ...
    def _get_icon_overrides(self, listitem, setToDefault=True, labelID=None):
        ...
        # If the icon is a VAR or an INFO, we're going to translate it and set
        # the untranslatedIcon property
        if icon.startswith("$"):
        ...
        # If the skin doesn't have the icon, replace it with
        # DefaultShortcut.png
        setDefault = False
        if (not xbmc.skinHasImage(icon) and setToDefault) and not iconIsVar:
            if oldicon is None:
                oldicon = icon
            setDefault = True
            icon = "DefaultShortcut.png"
        ...
        if setDefault and setToDefault:
            # We set this to the default icon, so we need to check if /that/
            # icon is overriden
    ...
    def _save_shortcuts(self):
        ...
        if self.changeMade:
            ...
            for listitem in self.allListItems:
                ...
                # If the item has a label or an action, or a specified property
                # from the override is present
                if common.try_decode(listitem.getLabel()) != LANGUAGE(32013) or listitem.getProperty(
                        "path") != "noop" or self.hasSaveWithProperty(listitem):
                    # Generate labelID, and mark if it has changed
                    labelID = listitem.getProperty("labelID")
                    newlabelID = labelID

                    # defaultID
                    defaultID = common.try_decode(
                    ...
                    newlabelID = DATA._get_labelID(
                        localLabel[3], listitem.getProperty("path"))
                    if self.group == "mainmenu":
                    ...
                    xmltree.SubElement(
                        shortcut, "label2").text = DATA.local(
                        listitem.getLabel2())[0]
                    ...
                    xmltree.SubElement(
                        shortcut, "icon").text = common.try_decode(icon)
                    xmltree.SubElement(
                        shortcut, "thumb").text = common.try_decode(thumb)

                    # Action
                    xmltree.SubElement(
                        shortcut, "action").text = common.try_decode(
                        listitem.getProperty("path"))

                    # Visible
                    if listitem.getProperty("visible-condition"):
                        xmltree.SubElement(
                            shortcut, "visible").text = listitem.getProperty("visible-condition")

                    # Disabled
                    if listitem.getProperty(
                            "skinshortcuts-disabled") == "True":
                        xmltree.SubElement(shortcut, "disabled").text = "True"

                    # Locked
                    if listitem.getProperty("LOCKED"):
                        xmltree.SubElement(
                            shortcut, "lock").text = listitem.getProperty("LOCKED")

                    # Additional properties
                    if listitem.getProperty("additionalListItemProperties"):
                        additionalProperties = eval(
            ...
            while not len(labelIDChanges) == 0:
                # Get the first labelID change, and check that we're not
                # changing anything from that
                ...
                if not len(labelIDChanges) == 1:
                    for x in range(1, len(labelIDChanges)):
                        if labelIDChanges[x][0] == labelIDTo:
                            ...
                            labelIDChanges.append(
                                [tempLocation, labelIDTo, defaultIDFrom])
                            labelIDTo = tempLocation
                            break

                # Make the change (0 - the main sub-menu, 1-5 - additional
                # submenus )
                for i in range(0, 6):
                    if i == 0:
                        groupName = labelIDFrom
                        paths = [[os.path.join(DATAPATH,
                                               DATA.slugify(labelIDFrom,
                                                            True) + ".DATA.xml").encode("utf-8"),
                                  "Move"],
                                 [os.path.join(SKINPATH,
                                               DATA.slugify(defaultIDFrom) + ".DATA.xml").encode("utf-8"),
                                  "Copy"],
                                 [os.path.join(DEFAULTPATH,
                                               DATA.slugify(defaultIDFrom) + ".DATA.xml").encode("utf-8"),
                                  "Copy"],
                                 [None,
                                  "New"]]
                        target = os.path.join(
                            DATAPATH, DATA.slugify(
                                labelIDTo, True) + ".DATA.xml").encode("utf-8")
                    else:
                        ...
                        paths = [
                            [
                                os.path.join(
                                    DATAPATH, DATA.slugify(
                                        "%s.%s" %
                                        (labelIDFrom, str(i)), True, isSubLevel=True) + ".DATA.xml").encode("utf-8"), "Move"], [
                                os.path.join(
                                    SKINPATH, DATA.slugify(
                                        "%s.%s" %
                                        (defaultIDFrom, str(i)), isSubLevel=True) + ".DATA.xml").encode("utf-8"), "Copy"], [
                                os.path.join(
                                    DEFAULTPATH, DATA.slugify(
                                        "%s.%s" %
                                        (defaultIDFrom, str(i)), isSubLevel=True) + ".DATA.xml").encode("utf-8"), "Copy"]]
                        target = os.path.join(
                            DATAPATH,
            ...
            xbmcgui.Window(10000).setProperty(
                "skinshortcuts-reloadmainmenu", "True")
    ...
    def _save_properties(self, properties, labelIDChanges, copyDefaults):
        ...
        if xbmcvfs.exists(path):
            ...
            for listProperty in listProperties:
                ...
                currentProperties.append([listProperty[0], listProperty[
                                         1], listProperty[2], listProperty[3]])
            ...
            if not currentProperty[0] == self.group:
                if currentProperty[0] in labelIDChanges.keys():
                    ...
                    if groupName in labelIDChanges.keys() and int(groupValue) in range(1, 6):
                        currentProperty[0] = "%s.%s" % (
                            labelIDChanges[groupName], groupValue)
        ...
        for singleProperty in properties:
            ...
            for toSave in singleProperty[1]:
                ...
                saveData.append(
                    [self.group, singleProperty[0], toSave[0], toSave[1]])
        ...
        # Clear saved properties in DATA, so it will pick up any new ones next
        # time we load a file
        DATA.currentProperties = None

    def _load_overrides(self):
        # Load various overrides from the skin, most notably backgrounds and
        # thumbnails
        ...
        # Are there any custom properties that shortcuts should be saved if
        # present
    ...
    def _load_overrides_context(self):
        ...
        for overrideType in ["skin", "script"]:
            ...
            for item in elem.findall("item"):
                ...
                self.contextItems.append(
                    (int(item.attrib.get("control")), condition, item.text))
    ...
    def _load_backgrounds_thumbnails(self):
        ...
        for elem in elems:
            ...
            if "icon" in elem.attrib:
                backgrounds.append(
                    [elem.attrib.get("icon"), DATA.local(elem.attrib.get('label'))[2]])
            elif elem.text.startswith("||BROWSE||"):
                # we want to include images from a VFS path...
                images = LIBRARY.getImagesFromVfsPath(
                    elem.text.replace("||BROWSE||", ""))
                for image in images:
                    backgrounds.append([image[0], image[1]])
            else:
                backgrounds.append(
                    [elem.text, DATA.local(elem.attrib.get('label'))[2]])
    ...
    def _load_customPropertyButtons(self):
        # Load a list of addition button IDs we'll handle for setting
        # additional properties
        ...
        for elem in tree.findall("propertySettings"):
            if "buttonID" in elem.attrib and "property" in elem.attrib:
                self.customPropertyButtons[
                    int(elem.attrib.get("buttonID"))] = elem.attrib.get("property")
            elif "buttonID" in elem.attrib and "toggle" in elem.attrib:
                self.customToggleButtons[
                    int(elem.attrib.get("buttonID"))] = elem.attrib.get("toggle")
    ...
    def onClick(self, controlID):
        ...
        if controlID in defaultControls:
            ...
            if xbmcgui.Window(10000).getProperty("skinshortcuts-loading") and int(calendar.timegm(
                    gmtime())) - int(xbmcgui.Window(10000).getProperty("skinshortcuts-loading")) <= 5:
                return

            common.log(
                "Launching management dialog for submenu/additional menu (" +
                str(controlID) +
                ")")
            xbmcgui.Window(10000).setProperty(
            ...
            launchGroup = self.getControl(
                211).getSelectedItem().getProperty("labelID")
            launchDefaultGroup = self.getControl(
                211).getSelectedItem().getProperty("defaultID")
            groupName = self.getControl(211).getSelectedItem().getLabel()

            if launchDefaultGroup is None:
                launchDefaultGroup = ""
            ...
            if launchGroup is None or launchGroup == "":
                ...
                for listitem in self.allListItems:
                    if listitem != orderIndex:
                        DATA._get_labelID(
                            listitem.getProperty("labelID"),
                            listitem.getProperty("path"))

                # Now generate labelID for this menu item, if it doesn't have
                # one
                labelID = self.getControl(211).getListItem(
                    num).getProperty("localizedString")
                if labelID is None or labelID == "":
                    launchGroup = DATA._get_labelID(
                        self.getControl(211).getListItem(num).getLabel(),
                        self.getControl(211).getListItem(num).getProperty("path"))
                else:
                    launchGroup = DATA._get_labelID(
                        labelID, self.getControl(211).getListItem(num).getProperty("path"))
                self.getControl(211).getListItem(
                    num).setProperty("labelID", launchGroup)
            ...
            # Check if 'level' property has been set
                ...
                launchGroup = launchGroup + "." + \
                    self.currentWindow.getProperty("level")
            ...
            ui = gui.GUI(
                ...
                "default",
                group=launchGroup,
                defaultGroup=launchDefaultGroup,
                groupLevel=launchLevel,
        ...
        if controlID in self.customToggleButtons:
            ...
            self.onClickToggleProperty(self.customToggleButtons[controlID])
        ...
        if controlID == 404 or controlID in self.customPropertyButtons:
            ...
            if self.currentWindow.getProperty("customProperty"):
                ...
                if self.currentWindow.getProperty("customValue"):
                    propertyValue = self.currentWindow.getProperty(
                        "customValue")
                    self.currentWindow.clearProperty("customValue")

                if propertyValue == "":
                    # No value set, so remove it from
                    # additionalListItemProperties
                ...
                if controlID == 404:
                    # Button 404, so we get the property from the window
                    # property
                    propertyName = self.currentWindow.getProperty(
                        "chooseProperty")
                ...
                for elem in tree.findall("propertySettings"):
                    ...
                    if "property" in elem.attrib and elem.attrib.get(
                            "property") == propertyName:
                        if "title" in elem.attrib:
                            dialogTitle = elem.attrib.get("title")
                        if "showNone" in elem.attrib and elem.attrib.get(
                                "showNone").lower() == "false":
                            showNone = False
                        if "imageBrowse" in elem.attrib and elem.attrib.get(
                                "imageBrowse").lower() == "true":
                            imageBrowse = True
                ...
                for elem in tree.findall("property"):
                    if "property" in elem.attrib and elem.attrib.get(
                            "property") == propertyName:
                        if "condition" in elem.attrib and not xbmc.getCondVisibility(
                        ...
                        if "label" in elem.attrib:
                            ...
                            if labelValue.startswith(
                                    "$INFO") or labelValue.startswith("$VAR") or labelValue.startswith( "$NUMBER" ):
                                propertyLabel.append(
                                    xbmc.getInfoLabel(labelValue))
                                propertyPretty.append(LIBRARY._create(
                                    ["", xbmc.getInfoLabel(labelValue), "", iconImage]))
                            else:
                                propertyLabel.append(DATA.local(labelValue)[2])
                                propertyPretty.append(LIBRARY._create(
                                    ["", labelValue, "", iconImage]))
                        else:
                            propertyLabel.append(DATA.local(foundProperty)[2])
                            propertyPretty.append(LIBRARY._create(
                                ["", foundProperty, "", iconImage]))

                # Show the dialog
                if usePrettyDialog:
                    w = library.ShowDialog(
                        "DialogSelect.xml",
                        CWD,
                        listing=propertyPretty,
                ...
                if browseSingle or browseMulti:
                    ...
                    if browseSingle:  # Single image
                        custom_image = imagedialog.browse(
                            2, xbmc.getLocalizedString(1030), 'files', '', True, False, None)
                    else:  # Multi-image
                        custom_image = imagedialog.browse(
                            0, xbmc.getLocalizedString(1030), 'files', '', True, False, None)

                    if custom_image:
                        self.changeMade = True
                        self._add_additionalproperty(
                            listitem, propertyName, custom_image)
                    else:
                        # User cancelled
                        return

            else:
                # The customProperty or chooseProperty window properties needs
                # to be set, so return
                self.currentWindow.clearProperty("customValue")
                return
    ...
    def onClickMoveUp(self):
        ...
        while True:
            ...
            self.allListItems[orderIndex - 1], self.allListItems[
                orderIndex] = self.allListItems[orderIndex], self.allListItems[orderIndex - 1]
            ...
            if self.allListItems[orderIndex].getProperty("visible-condition"):
                shouldBreak = xbmc.getCondVisibility(
                    self.allListItems[orderIndex].getProperty("visible-condition"))
    ...
    def onClickChangeThumbnail(self):
        ...
        custom_thumbnail = dialog.browse(2, xbmc.getLocalizedString(
            1030), 'files', '', True, False, self.thumbnailBrowseDefault)
    ...
    def onClickResetShortcuts(self):
        ...
        # Ask the user if they want to restore a shortcut, or reset to skin
        # defaults
        if self.alwaysReset:
            ...
            response = xbmcgui.Dialog().select(
                LANGUAGE(32102), [LANGUAGE(32103), LANGUAGE(32104)])
        ...
        if response == -1:
            ...
            for shortcut in shortcuts.getroot().findall("shortcut"):
                ...
                if item[1].getProperty("labelID") not in DATA.labelIDList:
                    restorePretty.append(LIBRARY._create(["", item[1].getLabel(), item[
                                         1].getLabel2(), {"icon": item[1].getProperty("icon")}]))
            ...
            # We now have our shortcut to return. Add it to self.allListItems
            # and labelID list
            self.allListItems.append(restoreItems[restoreShortcut])
            DATA.labelIDList.append(
    ...
    def onClickChooseBackground(self):
        ...
        # Create lists for the select dialog, with image browse buttons if
        # enabled
        if self.backgroundBrowse == "true":
            ...
            backgroundLabel = [
                ...
                LANGUAGE(32051),
                LANGUAGE(32052)]
            backgroundPretty = [LIBRARY._create(["",
                                                 LANGUAGE(32053),
                                                 "",
        ...
        defaultBackground = self.find_default(
            "background",
            selecteditem.getProperty("labelID"),
            selecteditem.getProperty("defaultID"))
        ...
        for key in self.backgrounds:
            if "::PLAYLIST::" in key[1]:
                for playlist in LIBRARY.widgetPlaylistsList:
                    ...
                    backgroundLabel.append(
                        key[1].replace(
                            "::PLAYLIST::",
                            playlist[1]))
                    backgroundPretty.append(LIBRARY._create(
                ...
                if xbmc.skinHasImage(key[0]) or virtualImage:
                    ...
                    backgroundPretty.append(
                        LIBRARY._create(["", label, "", {}]))

        if usePrettyDialog:
            w = library.ShowDialog(
                "DialogSelect.xml",
                CWD,
                listing=backgroundPretty,
        ...
        if selectedBackground == -1:
            ...
            self._remove_additionalproperty(
                selecteditem, "backgroundPlaylistName")
            self.changeMade = True
            return
            ...
            if selectedBackground == 1 and self.backgroundBrowse != "multi":  # Single image
                custom_image = imagedialog.browse(2, xbmc.getLocalizedString(
                    1030), 'files', '', True, False, self.backgroundBrowseDefault)
            else:  # Multi-image
                custom_image = imagedialog.browse(0, xbmc.getLocalizedString(
                    1030), 'files', '', True, False, self.backgroundBrowseDefault)

            if custom_image:
                self._add_additionalproperty(
                    selecteditem, "background", custom_image)
                self._add_additionalproperty(
                    selecteditem, "backgroundName", custom_image)
                self._remove_additionalproperty(
                    selecteditem, "backgroundPlaylist")
                self._remove_additionalproperty(
                    selecteditem, "backgroundPlaylistName")
            else:
                # User cancelled
            ...
            if isinstance(background[selectedBackground], list):
                ...
                self._add_additionalproperty(
                    selecteditem, "background", background[selectedBackground][0])
                self._add_additionalproperty(
                    selecteditem,
                    "backgroundName",
                ...
                self._add_additionalproperty(
                    selecteditem,
                    "backgroundPlaylistName",
                    background[selectedBackground][2])

            else:
                # User has selected a normal background
                self._add_additionalproperty(
                    selecteditem, "background", background[selectedBackground])
                self._add_additionalproperty(
                    selecteditem,
                    "backgroundName",
                    backgroundLabel[selectedBackground].replace(
    ...
    def onClickChooseThumbnail(self):
        ...
        if self.thumbnailNone:
            thumbnail.append("")
            thumbnailLabel.insert(0, LIBRARY._create(
                ["", self.thumbnailNone, "", {}]))
        ...
        for key in self.thumbnails:
            ...
            thumbnailLabel.append(LIBRARY._create(
                ["", key[1], "", {"icon": key[0]}]))
    ...
    def onClickWidgetSelect(self):
        ...
        if selecteditem.getProperty("widgetPath") == "":
            selectedShortcut = LIBRARY.selectShortcut(
                grouping="widget", showNone=True)
        else:
            selectedShortcut = LIBRARY.selectShortcut(
                grouping="widget",
                showNone=True,
                custom=True,
                currentAction=selecteditem.getProperty("widgetPath"))

        if selectedShortcut is None:
            # User cancelled
            return

        if selectedShortcut.getProperty(
                "Path") and selectedShortcut.getProperty("custom") == "true":
            # User has manually edited the widget path, so we'll update that
            # property only
            self._add_additionalproperty(
                selecteditem,
                "widgetPath" + widgetID,
                selectedShortcut.getProperty("Path"))

        elif selectedShortcut.getProperty("Path"):
            # User has chosen a widget

            # Let user edit widget title, if they want & skin hasn't disabled
            # it
            ...
            if self.widgetRename:
                ...
                keyboard = xbmc.Keyboard(
                    widgetTempName, xbmc.getLocalizedString(16105), False)
                keyboard.doModal()
            ...
            widgetPath = LIBRARY.addWidgetReload(
                selectedShortcut.getProperty("widgetPath"))
            ...
            self._add_additionalproperty(
                selecteditem, "widgetName" + widgetID, widgetName)
            self._add_additionalproperty(
                selecteditem,
                "widgetType" + widgetID,
                selectedShortcut.getProperty("widgetType"))
            self._add_additionalproperty(
                selecteditem,
                "widgetTarget" + widgetID,
                selectedShortcut.getProperty("widgetTarget"))
            self._add_additionalproperty(
                selecteditem, "widgetPath" + widgetID, widgetPath)
            if self.currentWindow.getProperty(
                    "useWidgetNameAsLabel") == "true" and widgetID == "":
                self._set_label(
                    selecteditem,
                    selectedShortcut.getProperty(
                        ("widgetName")))
            ...
            self._remove_additionalproperty(
                selecteditem, "widgetName" + widgetID)
            self._remove_additionalproperty(
                selecteditem, "widgetType" + widgetID)
            self._remove_additionalproperty(
                selecteditem, "widgetTarget" + widgetID)
            self._remove_additionalproperty(
                selecteditem, "widgetPath" + widgetID)
            if self.currentWindow.getProperty(
                    "useWidgetNameAsLabel") == "true" and widgetID == "":
                self._set_label(
                    selecteditem,
    ...
    def onClickToggleProperty(self, propertyName):
        common.log(" - Toggle custom property")

        # Get currently selected item
        control, num, selecteditem, orderIndex = self.onClickCurrentItem()

        if selecteditem.getProperty(propertyName) == "True":
            self._remove_additionalproperty(selecteditem, propertyName)
        else:
            self._add_additionalproperty(selecteditem, propertyName, "True")
    ...
    def onClickSelectShortcut(self):
        ...
        num = self.getControl(211).getSelectedPosition()
        orderIndex = int(self.getControl(211).getListItem(
        ...
        if selectedShortcut is not None:
            listitemCopy = self._duplicate_listitem(
                selectedShortcut, selecteditem)
            ...
            if selectedShortcut.getProperty("smartShortcutProperties"):
                for listitemProperty in eval(
                        selectedShortcut.getProperty("smartShortcutProperties")):
                    self._add_additionalproperty(
                        listitemCopy, listitemProperty[0], listitemProperty[1])
            ...
            if defaultBackground:
                self._add_additionalproperty(
                    listitemCopy, "background", defaultBackground["path"])
                self._add_additionalproperty(
                    listitemCopy, "backgroundName", defaultBackground["label"])

            # set default widget for this item (if any)
            defaultWidget = self.find_defaultWidget(
                listitemCopy.getProperty("labelID"),
                listitemCopy.getProperty("defaultID"))
            if defaultWidget:
                self._add_additionalproperty(
                    listitemCopy, "widget", defaultWidget["widget"])
                self._add_additionalproperty(
                    listitemCopy, "widgetName", defaultWidget["name"])
                self._add_additionalproperty(
                    listitemCopy, "widgetType", defaultWidget["type"])
                self._add_additionalproperty(
                    listitemCopy, "widgetPath", defaultWidget["path"])
                self._add_additionalproperty(
                    listitemCopy, "widgetTarget", defaultWidget["target"])
    ...
    def _duplicate_listitem(self, listitem, originallistitem=None):
        ...
        listitemCopy = xbmcgui.ListItem(
            label=listitem.getLabel(),
            label2=listitem.getLabel2(),
            iconImage=listitem.getProperty("icon"),
            thumbnailImage=listitem.getProperty("thumbnail"))
        ...
        listitemCopy.setProperty(
            "localizedString",
            listitem.getProperty("localizedString"))
        listitemCopy.setProperty(
            "shortcutType",
            listitem.getProperty("shortcutType"))
        listitemCopy.setProperty(
            "skinshortcuts-disabled",
        ...
        if listitem.getProperty("defaultID"):
            ...
            listitemCopy.setProperty(
                "defaultID", listitem.getProperty("labelID"))
        else:
            listitemCopy.setProperty(
                "defaultID",
                DATA._get_labelID(
                    DATA.local(
                        listitem.getProperty("localizedString"))[3],
                    listitem.getProperty("path"),
                    True))
        ...
        # Revert to original icon (because we'll override it again in a
        # minute!)
        if listitem.getProperty("original-icon"):
            icon = listitem.getProperty("original-icon")
            if icon == "":
        ...
        if originallistitem is None:
            ...
            # in tact)
            listitemCopy.setProperty(
                "labelID", originallistitem.getProperty("labelID"))
    ...
    def _add_additionalproperty(self, listitem, propertyName, propertyValue):
        ...
        properties = []
        if listitem.getProperty("additionalListItemProperties"):
            properties = eval(listitem.getProperty(
                "additionalListItemProperties"))

        foundProperty = False
        for singleProperty in properties:
            if singleProperty[0] == propertyName:
                foundProperty = True
                singleProperty[1] = DATA.local(propertyValue)[0]

        if not foundProperty:
            properties.append([propertyName, DATA.local(propertyValue)[0]])

        # translate any INFO labels (if needed) so they will be displayed
        # correctly in the gui
        if propertyValue:
            if propertyValue.startswith(
                    "$") and not propertyValue.startswith("$SKIN"):
                listitem.setProperty(
                    propertyName, xbmc.getInfoLabel(propertyValue))
            else:
                listitem.setProperty(
                    propertyName, DATA.local(propertyValue)[2])
    ...
    def _remove_additionalproperty(self, listitem, propertyName):
        ...
        for property in properties:
            if property[
                    0] == propertyName or "%s-NUM" % (property[0]) == "%s-NUM" % (propertyName):
    ...
    def find_defaultWidget(self, labelID, defaultID):
        ...
        defaultWidget = self.find_default(
            "widgetdefaultnode", labelID, defaultID)
    ...
    def find_default(self, backgroundorwidget, labelID, defaultID):
        ...
        if elems is not None:
            for elem in elems:
                if elem.attrib.get("labelID") == labelID or elem.attrib.get(
                        "defaultID") == defaultID:
                    if "group" in elem.attrib:
                        if elem.attrib.get("group") == self.group:
                            if backgroundorwidget == "widgetdefaultnode":
                                # if it's a widgetdefaultnode, return the whole
                                # element
                                return elem
                            else:
                                return elem.text
