...
def kodiwalk(path, stringForce=False):
    json_query = xbmc.executeJSONRPC(
        '{"jsonrpc":"2.0","method":"Files.GetDirectory","params":{"directory":"%s","media":"files"},"id":1}' %
        str(path))
    json_query = unicode(json_query, 'utf-8', errors='ignore')
    json_response = json.loads(json_query)
    files = []
    if 'result' in json_response and 'files' in json_response[
            'result'] and json_response['result']['files'] is not None:
        for item in json_response['result']['files']:
            if 'file' in item and 'filetype' in item and 'label' in item:
                if item['filetype'] == 'directory' and not item[
                        'file'].endswith(('.xsp', '.m3u', '.xml/', '.xml')):
                    if stringForce and item['file'].startswith(stringForce):
                        files = files + \
                            kodiwalk(xbmc.translatePath(item['file']), stringForce)
                    else:
                        files = files + kodiwalk(item['file'], stringForce)
                else:
                    if stringForce and item['file'].startswith(stringForce):
                        files.append({'path': xbmc.translatePath(
                            item['file']), 'label': item['label']})
                    else:
                        files.append(
                            {'path': item['file'], 'label': item['label']})
    return files
...
class LibraryFunctions():
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.widgetProviders = [["service.library.data.provider",
                                 None,
    ...
    def loadLibrary(self, library):
        ...
        if self.loaded[library][0] is True:
            ...
            for i in range(0, 50):
                if xbmc.Monitor().waitForAbort(
                        0.1) or self.loaded[library][0] is True:
                    return True
        else:
            # We're going to populate the list
            self.loaded[library][0] = "Loading"

        # Call the function responsible for loading the library type we've been
        # passed
        ...
        try:
            if library == "static":
                ...
                xbmc.executeJSONRPC(
                    '{ "jsonrpc": "2.0", "id": 0, "method": "Files.GetDirectory", "params": { "directory": "upnp://", "media": "files" } }')
        ...
        for subnode in tree:
            ...
            if "condition" in subnode.attrib:
                if not xbmc.getCondVisibility(subnode.attrib.get("condition")):
                    number += 1
                    continue
            if "installWidget" in subnode.attrib and subnode.attrib.get(
                    "installWidget").lower() == "true":
    ...
    def buildNodeListing(self, nodes):
        ...
        for node in nodes:
            ...
            if node.tag == "shortcut":
                shortcutItem = self._create([node.text, node.attrib.get(
                    "label"), node.attrib.get("type"), {"icon": node.attrib.get("icon")}])
                if "widget" in node.attrib:
                    # This is a widget shortcut, so add the relevant widget
                    # information
                    shortcutItem.setProperty(
                        "widget", node.attrib.get("widget"))
                    if "widgetName" in node.attrib:
                        shortcutItem.setProperty(
                            "widgetName", node.attrib.get("widgetName"))
                    else:
                        shortcutItem.setProperty(
                            "widgetName", node.attrib.get("label"))
                    shortcutItem.setProperty("widgetPath", node.text)
                    if "widgetType" in node.attrib:
                        shortcutItem.setProperty(
                            "widgetType", node.attrib.get("widgetType"))
                    else:
                        shortcutItem.setProperty("widgetType", "")
                    if "widgetTarget" in node.attrib:
                        shortcutItem.setProperty(
                            "widgetTarget", node.attrib.get("widgetTarget"))
                    else:
    ...
    def retrieveContent(self, content):
        if content == "upnp-video":
            items = [self._create(["||UPNP||", "32070", "32069", {
                                  "icon": "DefaultFolder.png"}])]
        elif content == "upnp-music":
            items = [self._create(["||UPNP||", "32070", "32073", {
                                  "icon": "DefaultFolder.png"}])]
            ...
            for elem in tree.findall("shortcut"):
                if "grouping" in elem.attrib:
                    if group == elem.attrib.get("grouping"):
                        ...
                        groupType = elem.attrib.get("type")
                        ...
                        listitem = self._create(
                            [action, label, groupType, {"icon": icon, "thumb": thumb}])
                    ...
                    if groupType is None or groupType == "":
                        groupType = "Skin Provided"
    ...
    def _create(self, item, allowOverrideLabel=True):
        ...
        if item[0].startswith("||"):
            ...
            # causes issues with some folders picking up overriden icons
            # incorrectly
            noNonLocalized = True
        ...
        if self.useDefaultThumbAsIcon and thumbnail is not None:
            icon = thumbnail
            thumbnail = None
        ...
        # DefaultShortcut.png
        if (not displayIcon or not xbmc.skinHasImage(
                displayIcon)) and not iconIsVar:
    ...
    def _get_icon_overrides(self, tree, item, content, setToDefault=True):
        ...
        setDefault = False
        if not xbmc.skinHasImage(newicon) and setToDefault:
            oldicon = item.getProperty("icon")
            newicon = "DefaultShortcut.png"
            setDefault = True
    ...
    def _parse_libraryNodes(self, library, nodeType):
        ...
        for key in nodes:
            ...
            # make sure the path ends with a trailing slash te prevent weird
            # kodi behaviour
            if "/" in nodes[key][2] and not nodes[key][2].endswith("/"):
                nodes[key][2] += "/"

            if nodes[key][3] == "folder":
                item = self._create(["%s%s" % (action, nodes[key][2]), nodes[key][
                                    0], nodes[key][3], {"icon": nodes[key][1]}])
            elif nodes[key][3] == "grouped":
                item = self._create(["%s%s" % (action, nodes[key][2]), nodes[key][
                                    0], nodes[key][3], {"icon": nodes[key][1]}])
            else:
                item = self._create(["ActivateWindow(%s,%s,return)" % (windowID, nodes[key][
                                    2]), nodes[key][0], nodes[key][3], {"icon": nodes[key][1]}])
    ...
    def staticShortcuts(self):
        ...
        staticGroups = [("common", "32034"), ("commands", "32054"),
                        ("settings", "10004"), ("pvr-tv", "32017"), ("pvr-radio", "32087")]

        # Pull out static shortcuts
        trees = [
            DATA._get_overrides("skin").find("static"),
            DATA._get_overrides("script").find("static")]
        ...
        for staticGroup in staticGroups:
            ...
            for tree in trees:
                ...
                for staticItem in staticItems.findall("item"):
                    if "condition" in staticItem.attrib and not xbmc.getCondVisibility(
                            staticItem.attrib.get("condition")):
                        # Condition specified doesn't match
                        continue

                    # Build static item
                    listitems.append(self._create([staticItem.text, staticItem.attrib.get(
                        "label"), staticGroup[1], {"icon": staticItem.attrib.get("icon")}]))
    ...
    def librarysources(self):
        for mediaType in [
            ...
            listitems = []
            json_query = xbmc.executeJSONRPC(
                '{ "jsonrpc": "2.0", "id": 0, "method": "Files.GetSources", "params": { "media": "%s" } }' %
                (mediaType[0]))
            json_query = unicode(json_query, 'utf-8', errors='ignore')
            json_response = json.loads(json_query)

            # Add all directories returned by the json query
            if 'result' in json_response and 'sources' in json_response[
                    'result'] and json_response['result']['sources'] is not None:
                for item in json_response['result']['sources']:
                    listitems.append(self._create(
                        ["||SOURCE||" + item['file'], item['label'], "32069", {"icon": "DefaultFolder.png"}]))
            ...
            for file in kodiwalk(path[0]):
                try:
                    ...
                    if playlist.endswith('.xsp'):
                        ...
                        xmldata = xmltree.fromstring(
                            contents_data.encode('utf-8'))
                        mediaType = "unknown"
                        for line in xmldata.getiterator():
                            ...
                            if line.tag == "name" and mediaLibrary is not None:
                                ...
                                if not name:
                                    name = label
                                # Create a list item
                                listitem = self._create(["::PLAYLIST>%s::" % (mediaLibrary), name, path[
                                                        1], {"icon": "DefaultPlaylist.png"}])
                                listitem.setProperty(
                                    "action-play", "PlayMedia(" + playlist + ")")
                                listitem.setProperty(
                                    "action-show",
                                    "ActivateWindow(" +
                                    mediaLibrary +
                        ...
                        name = label
                        listitem = self._create(["::PLAYLIST>%s::" % (path[2]), name, path[
                                                1], {"icon": "DefaultPlaylist.png"}])
                        listitem.setProperty(
                            "action-play", "PlayMedia(" + playlist + ")")
                        listitem.setProperty(
                            "action-show", "ActivateWindow(%s,%s,return)".encode('utf-8') %
                            (path[2], playlist))
                        listitem.setProperty(
                            "action-party",
                            "PlayerControl(PartyMode(%s))" %
    ...
    def favourites(self):
        ...
        for favourite in listing:
            ...
            if ('RunScript' not in path) and (
                    'StartAndroidActivity' not in path) and not (path.endswith(',return)')):
                path = path.rstrip(')')
                path = path + ',return)'
            ...
            listitems.append(self._create(
                [path, name, "32006", {"icon": "DefaultFolder.png", "thumb": thumb}]))
    ...
    def addons(self):
        ...
        for contenttype, listitems, addonType in contenttypes:
            ...
            json_query = xbmc.executeJSONRPC(
                '{ "jsonrpc": "2.0", "id": 0, "method": "Addons.Getaddons", "params": { "type": "%s", "properties": ["name", "path", "thumbnail", "enabled"] } }' %
                addonType)
            json_query = unicode(json_query, 'utf-8', errors='ignore')
            json_response = json.loads(json_query)
            ...
            if 'result' in json_response and 'addons' in json_response[
                    'result'] and json_response['result']['addons'] is not None:
                for item in json_response['result']['addons']:
                    if item['enabled']:
                        path = "RunAddOn(" + \
                            item['addonid'].encode('utf-8') + ")"
                        action = None
                        ...
                        if item['thumbnail'] != "":
                            ...
                            thumb = None
                        listitem = self._create([path, item['name'], shortcutType, {
                        ...
                        if item["type"] == "xbmc.python.pluginsource":
                            ...
                            # If its executable or a game, save it to our
                            # program plugin widget list
                            if contenttype in ("executable", "game"):
                                plugins[item["name"]] = listitem

                        elif contenttype == "executable":
                            # Check if it's a program that can be run as an
                            # exectuble
                            provides = self.parseAddonXML(
                                item["path"], "plugin")
                            for content in provides:
                                # For each content that it provides, add it to
                                # the add-ons for that type
                                contentData = {
                                    "video": [
                                        "::SCRIPT::32010", videoItems], "audio": [
                                        "::SCRIPT::32011", audioItems], "image": [
                                        "::SCRIPT::32012", imageItems], "executable": [
                                        "::SCRIPT::32009", executableItems]}
                                if content in contentData:
                                    # Add it as a plugin in the relevant
                                    # category
                                    otherItem = self._create([path, item['name'] + "  >", contentData[content][
                                                             0], {"icon": "DefaultAddon.png", "thumb": thumb}])
                                    otherItem.setProperty(
            ...
            if contenttype == "executable":
                self.addToDictionary(
                    "addon-program", self.sortDictionary(listitems))
                self.addToDictionary(
                    "addon-program-plugin",
                    self.sortDictionary(plugins))
                common.log(
                    " - %i programs found (of which %i are plugins)" %
                    (len(listitems), len(plugins)))
            elif contenttype == "video":
                self.addToDictionary(
                    "addon-video", self.sortDictionary(listitems))
                common.log(" - " +
                           str(len(listitems)) +
                           " video add-ons found")
            elif contenttype == "audio":
                self.addToDictionary(
                    "addon-audio", self.sortDictionary(listitems))
                common.log(" - " +
                           str(len(listitems)) +
                           " audio add-ons found")
            elif contenttype == "image":
                self.addToDictionary(
                    "addon-image", self.sortDictionary(listitems))
                common.log(" - " +
                           str(len(listitems)) +
                           " image add-ons found")
            elif contenttype == "game":
                self.addToDictionary(
                    "addon-game", self.sortDictionary(listitems))
                self.addToDictionary(
                    "addon-game-plugin",
                    self.sortDictionary(plugins))
                common.log(
                    " - %i game add-ons found (of which %i are plugins)" %
                    (len(listitems), len(plugins)))

    def parseAddonXML(self, path, addonType):
        ...
        try:
            ...
            for extension in tree.findall("extension"):
                if "point" in extension.attrib and extension.attrib.get("point") == info[
                        addonType][0]:
                    # Find out what content type it provides
                    details = extension.find(info[addonType][1])
                    if details is None:
                        return []
                    if addonType == "plugin":
    ...
    def detectPluginContent(self, item):
        ...
        if "showtitle" not in item and "artist" not in item:
            ...
            # if it's missing it means this is a main directory listing and no
            # need to scan the underlying listitems.
            return None
        ...
        if "showtitle" not in item and "artist" in item:
            ...
            if (item["showtitle"] and not item["artist"]):
                ...
                if item["type"] == "season" or (
                        item["season"] > -1 and item["episode"] == -1):
                    return "seasons"
    ...
    def widgets(self):
        ...
        for elem in elems:
            ...
            listitem = self._create([elem.text, DATA.local(elem.attrib.get('label'))[
                                    2], "::SCRIPT::32099", {"icon": widgetIcon}])
            listitem.setProperty("widget", elem.text)
            if widgetName is not None:
                listitem.setProperty("widgetName", widgetName)
            else:
                listitem.setProperty(
                    "widgetName", DATA.local(
                        elem.attrib.get('label'))[2])
    ...
    def explorer(
        ...
        if len(label) != 1:
            dialogLabel = common.try_decode(label[0]).replace(
                "  >", "") + " - " + common.try_decode(label[-1]).replace("  >", "")
        ...
        if not "script.skin.helper.service" in location:
            ...
            if isWidget:
                createLabel = "32100"
            listings.append(self._get_icon_overrides(
                tree, self._create(["::CREATE::", createLabel, "", {}]), ""))

        common.log(
            "Getting %s - %s" %
            (dialogLabel, common.try_decode(location)))

        # Show a waiting dialog, then get the listings for the directory
        dialog = xbmcgui.DialogProgress()
        dialog.create(dialogLabel, LANGUAGE(32063))

        # we retrieve a whole bunch of properties, needed to guess the content
        # type properly
        json_query = xbmc.executeJSONRPC(
            '{ "jsonrpc": "2.0", "id": 0, "method": "Files.GetDirectory", "params": { "properties": ["title", "file", "thumbnail", "episode", "showtitle", "season", "album", "artist", "imdbnumber", "firstaired", "mpaa", "trailer", "studio", "art"], "directory": "' +
            location +
            '", "media": "files" } }')
        json_query = unicode(json_query, 'utf-8', errors='ignore')
        json_response = json.loads(json_query)
        ...
        # Add all directories returned by the json query
        if 'result' in json_response and 'files' in json_response[
                'result'] and json_response['result']['files']:
            ...
            for item in json_result:
                ...
                if location.startswith("library://"):
                    ...
                    if item["filetype"] == "directory":
                        ...
                        listitem = self._create(["ActivateWindow(%s,%s,return)" % (windowID, item[
                                                "file"]), altLabel, "", {"icon": "DefaultFolder.png", "thumb": thumb}])

                        if item["file"].endswith(
                                ".xml/") and NODE.isGrouped(item["file"]):
                            listitem = self._create([item["file"], "%s  >" % (item["label"]), "", {
                                                    "icon": "DefaultFolder.png", "thumb": thumb}])

                        # Add widget properties
                        widgetName = common.try_decode(
                            label[0]).replace(
                            "  >", "") + " - " + item["label"]
                        listitem.setProperty("widget", "Library")
                        listitem.setProperty("widgetName", widgetName)
                        ...
                        listings.append(
                            self._get_icon_overrides(
                                tree, listitem, ""))

                # some special code for smart shortcuts in
                # script.skin.helper.service
                    ...
                    listitem = self._create([item["file"], altLabel, "", {
                        "icon": item.get("icon"), "thumb": thumb}])
                    # add all passed properties to the gui to set default
                    # background, widget etc.
                    properties = []
                    for key, value in smartShortCutsData.iteritems():
                        properties.append([key, value])
                    listitem.setProperty(
                        "smartShortcutProperties", repr(properties))
                    listitem.setProperty("untranslatedIcon", thumb)
                    listitem.setProperty(
                        "widget", smartShortCutsData.get(
                            "widget", "Addon"))
                    listitem.setProperty("widgetName", item["label"])
                    listitem.setProperty(
                        "widgetType", smartShortCutsData["type"])
                    if smartShortCutsData["type"] == "music" or smartShortCutsData[
                            "type"] == "artists" or smartShortCutsData["type"] == "albums" or smartShortCutsData["type"] == "songs":
                        listitem.setProperty("widgetTarget", "music")
                    else:
                        listitem.setProperty("widgetTarget", "videos")
                    listitem.setProperty(
                        "widgetPath", smartShortCutsData["list"])
                    listings.append(
                        self._get_icon_overrides(
                            tree, listitem, ""))
                    ...
                    if item["filetype"] == "directory":
                        ...
                        if item["thumbnail"] is not "":
                            thumb = item["thumbnail"]
                        listitem = self._create([item["file"], item[
                                                "label"] + "  >", "", {"icon": "DefaultFolder.png", "thumb": thumb}])
                        listings.append(
                            self._get_icon_overrides(
                                tree, listitem, ""))
                    else:
        ...
        if selectedItem == -2:
            ...
            return self._explorer_install_widget_provider(
                history, history[len(history) - 1], label, thumbnail, itemType, isWidget)
            ...
            if selectedAction == "::CREATE::":
                ...
                if itemType in ["32010", "32014", "32069"]:
                    ...
                    action = 'ActivateWindow(Pictures,"' + \
                        location + '",return)'
                    ...
                    action = 'ActivateWindow(Programs,"' + \
                        location + '",return)'
                ...
                # User is going up the heirarchy, remove current level and
                ...
                # The user wants to create a shortcut to a specific shortcut
                # listed
                listitem = listings[selectedItem]

                # Add widget details
                if isLibrary:
                    widgetType = NODE.get_mediaType(
                        listitem.getProperty("widgetPath"))
                    if widgetType != "unknown":
                        listitem.setProperty("widgetType", widgetType)
                ...
                # User has chosen a sub-level to display, add details and
                # re-call this function
                history.append(selectedAction)
                label.append(listings[selectedItem].getLabel())
                thumbnail.append(
                    listings[selectedItem].getProperty("thumbnail"))
                return self.explorer(
                    history,
                    selectedAction,
                    label,
    ...
    def _explorer_install_widget_provider(
            self,
            history,
            location,
            label,
        ...
        # The user has clicked the 'Get More...' button to install additional
        # widget providers
        providerList = []
        providerLabel = []
        ...
        for widgetProvider in self.widgetProviders:
            if widgetProvider[1] is None or xbmc.getCondVisibility(
                    widgetProvider[1]):
    ...
    def _select_install_widget_provider(
            self,
            group,
            grouping,
            custom,
    ...
    def _allow_install_widget_provider(
            self, location, isWidget, nodeAllows=None):
    ...
    def getImagesFromVfsPath(self, path):
        ...
        images = []
        json_query = xbmc.executeJSONRPC(
            '{ "jsonrpc": "2.0", "id": 0, "method": "Files.GetDirectory", "params": { "properties": ["title", "art", "file", "fanart"], "directory": "' +
            path +
            '", "media": "files" } }')
        json_query = unicode(json_query, 'utf-8', errors='ignore')
        json_response = json.loads(json_query)
        if 'result' in json_response and 'files' in json_response[
            ...
            for item in json_result:
                ...
                if item.get(
                        "file",
                        None).lower().endswith(".jpg") or item.get(
                        "file",
                        None).lower().endswith(".png"):
                    image = item["file"]
...
# =====================================
    ...
    def selectShortcut(
        ...
        if showNone is not False and group == "":
            availableShortcuts.insert(0, self._create(
                ["::NONE::", LANGUAGE(32053), "", {"icon": "DefaultAddonNone.png"}]))

        if custom is not False and group == "":
            availableShortcuts.append(self._create(
                ["||CUSTOM||", LANGUAGE(32024), "", {}]))

        if group != "":
            # Add a link to go 'up'
            availableShortcuts.insert(
                0, self._create(["::BACK::", "..", "", {}]))
        ...
        if number == -2:
            # Get more button
            common.log("Selected get more button")
            return self._select_install_widget_provider(
                group, grouping, custom, showNone, currentAction)
            ...
            if path.startswith("||NODE||"):
                ...
                selectedShortcut = self.explorer(
                    [
                        ...
                        "||VIDEO||", ""), [
                        selectedShortcut.getLabel()], [
                        selectedShortcut.getProperty("thumbnail")], "32014", isWidget=isWidget)
                # Convert backslashes to double-backslashes (windows fix)
                if selectedShortcut is not None:
                    newAction = selectedShortcut.getProperty("Path")
                    newAction = newAction.replace("\\", "\\\\")
                ...
                selectedShortcut = self.explorer(
                    [
                        ...
                        "||AUDIO||", ""), [
                        selectedShortcut.getLabel()], [
                        selectedShortcut.getProperty("thumbnail")], "32019", isWidget=isWidget)
                # Convert backslashes to double-backslashes (windows fix)
                if selectedShortcut is not None:
                    newAction = selectedShortcut.getProperty("Path")
                    newAction = newAction.replace("\\", "\\\\")
                ...
                selectedShortcut = self.explorer([path.replace("||SOURCE||", "")], path.replace("||SOURCE||", ""), [selectedShortcut.getLabel()], [
                                                 selectedShortcut.getProperty("thumbnail")], selectedShortcut.getProperty("shortcutType"), isWidget=isWidget)
                if selectedShortcut is None or "upnp://" in selectedShortcut.getProperty(
                        "Path"):
                ...
                if isWidget:
                    ...
                    selectedShortcut.setProperty(
                        "Path", selectedShortcut.getProperty("Path").replace(
                            "\\", "\\\\"))
                    selectedShortcut.setProperty(
                        "displayPath",
                        selectedShortcut.getProperty("Path").replace(
                            "\\",
                            "\\\\"))
                ...
                if isWidget:
                    ...
                    selectedShortcut.setProperty(
                        "chosenPath", selectedShortcut.getProperty("action-show"))
                elif not ">" in path or "Videos" in path:
                    # Give the user the choice of playing or displaying the
                    # playlist
                    dialog = xbmcgui.Dialog()
                    userchoice = dialog.yesno(LANGUAGE(32040), LANGUAGE(
                        32060), "", "", LANGUAGE(32061), LANGUAGE(32062))
                    # False: Display
                    # True: Play
                    if not userchoice:
                        selectedShortcut.setProperty(
                        ...
                        selectedShortcut.setProperty(
                            "chosenPath", selectedShortcut.getProperty("action-play"))
                elif ">" in path:
                    # Give the user the choice of playing, displaying or party
                    ...
                    if not userchoice or userchoice == 0:
                        ...
                        selectedShortcut.setProperty(
                            "chosenPath", selectedShortcut.getProperty("action-party"))
                ...
                if (keyboard.isConfirmed()):
                    ...
                    if action != "":
                        ...
                        selectedShortcut = xbmcgui.ListItem(
                            None, LANGUAGE(32024))
...
# ==============================
    ...
    # a widgetReload property - managed by Skin Helper Service - to plugins to
    # help display updated content

    def addWidgetReload(self, widgetPath):
        if "plugin://" not in widgetPath or "reload=" in widgetPath.lower(
        ) or "script.extendedinfo" in widgetPath.lower():
            # Not a plugin, or already has a reload parameter
            # Also return on Extended Info, as it doesn't like its parameters
            # to be altered
            return widgetPath
        ...
        # And return it all
        return "%s%sreload=$INFO[Window(Home).Property(widgetreload)]" % (
            widgetPath, reloadParameter)
