...
class DataFunctions():
    ...
    def _get_addon_labelID(self, action):
        # This will check the action to see if this is a program or the root of
        # a plugin and, if so, return that as the labelID
    ...
    def _get_shortcuts(
        ...
        # Additionally, if the override files haven't been loaded, we'll load
        # them too
        common.log("Loading shortcuts for group " + group)

        if profileDir is None:
            profileDir = xbmc.translatePath(
                "special://profile/").decode("utf-8")

        userShortcuts = os.path.join(
            profileDir, "addon_data", ADDONID, self.slugify(
                group, True, isSubLevel=isSubLevel) + ".DATA.xml")
        skinShortcuts = os.path.join(
            SKINPATH, self.slugify(group) + ".DATA.xml")
        defaultShortcuts = os.path.join(
            DEFAULTPATH, self.slugify(group) + ".DATA.xml")
        if defaultGroup is not None:
            skinShortcuts = os.path.join(
                SKINPATH, self.slugify(defaultGroup) + ".DATA.xml")
            defaultShortcuts = os.path.join(
                DEFAULTPATH, self.slugify(defaultGroup) + ".DATA.xml")
    ...
    def _process_shortcuts(
        ...
        for node in tree.getroot().findall("shortcut"):
            # If not user shortcuts, remove locked nodes (in case of naughty
            # skinners!)
            if not isUserShortcuts:
            ...
            for override in allGroupOverrides:
                if override.attrib.get("group") == group:
                    ...
                    newaction.set(
                        "condition", override.attrib.get("condition"))

            # Generate the labelID
            labelID = self._get_labelID(
                self.local(
                    node.find("label").text)[3].replace(
                    " ", "").lower(), action.text)
            ...
            additionalProperties = self.checkAdditionalProperties(
                group, labelID, defaultID, isUserShortcuts, profileDir)
            ...
            xmltree.SubElement(
                node, "additional-properties").text = repr(additionalProperties)
            ...
            overridenIcon = self._get_icon_overrides(
            ...
            if visibilityCondition != "":
                ...
                for override in skinoverrides.findall("visibleoverride"):
                    if override.attrib.get("condition").lower(
                    ) != visibilityCondition.lower():
                        # Not overriding this visibility condition
                        continue

                    if "group" in override.attrib and not override.attrib.get(
                            "group") == group:
                    ...
                    # It's overriden - add the original action with the
                    # visibility condition
                    originalAction = xmltree.SubElement(
                        node, "override-visibility")
                    originalAction.text = action.text
                    originalAction.set("condition", visibilityCondition)

                    # And add the new action with the inverse visibility
                    # condition
            ...
            for overrideTree in overrideTrees:
                ...
                if overrideTree is not None:
                    for elem in overrideTree.findall("override"):
                        # Pull out the current action, and any
                        # already-overriden actions
                        itemsToOverride = []
                        for itemToOverride in node.findall(
                                "override-visibility"):
                        ...
                        for itemToOverride in itemsToOverride:
                            ...
                            if (
                                elem.attrib.get("action") == itemToOverride.text and (
                                ...
                                for actions in elem.findall("action"):
                                    newaction = xmltree.SubElement(
                                        node, "override-action")
                                    if "::ACTION::" in actions.text:
                                        newaction.text = actions.text.replace(
                                            "::ACTION::", itemToOverride.text)
                                    else:
                                        newaction.text = actions.text
                                    if overrideVisibility is not None:
                                        newaction.set(
                                            "condition", overrideVisibility)
                                ...
                                if newaction is not None and itemToOverride.get(
                                        "condition"):
                                    newaction.set(
                                        "condition", "[%s] + [%s]" %
            ...
            for elem in node.findall("override-visibility"):
                if elem.get("overriden") == "True":
                    ...
                    # The item hasn't been overriden, so change it to an
                    # override-action element
            ...
            for elem in skinoverrides.findall("shortcut"):
                if elem.text == action.text and "condition" in elem.attrib:
                    if not visibilityNode:
                        xmltree.SubElement(
                            node, "visibility").text = elem.attrib.get("condition")
                    else:
            ...
            if additionalVisibility is not None:
                if visibilityNode is None:
                    xmltree.SubElement(
                        node, "visibility").text = additionalVisibility.text
                else:
                    visibilityNode.text = "[" + visibilityNode.text + \
    ...
    def _get_skin_required(self, listitems, group, profileDir):
        ...
        # Once the tree is built, it sends them to _process_shortcuts for any
        # overrides, etc, then adds them to the menu tree
        ...
        for elem in tree.findall("requiredshortcut"):
            if elem.text not in actions:
                ...
                requiredShortcut = xmltree.SubElement(
                    listitems.getroot(), "shortcut")

                # Label and label2
                xmltree.SubElement(
                    requiredShortcut,
                    "label").text = elem.attrib.get("label")
                xmltree.SubElement(
                    requiredShortcut,
                    "label2").text = xbmc.getSkinDir()

                # Icon and thumbnail
                if "icon" in elem.attrib:
                    xmltree.SubElement(
                        requiredShortcut,
                        "icon").text = elem.attrib.get("icon")
                else:
                    xmltree.SubElement(
                        requiredShortcut,
                        "icon").text = "DefaultShortcut.png"
                if "thumb" in elem.attrib:
                    xmltree.SubElement(
                        requiredShortcut,
                        "thumb").text = elem.attrib.get("thumbnail")
                ...
                xmltree.SubElement(
                    requiredShortcut,
                    "lock").text = xbmc.getSkinDir()

    def _get_icon_overrides(
            self,
            tree,
            icon,
            group,
        ...
        if not xbmc.skinHasImage(newicon.encode("utf-8")) and setToDefault:
            newicon = self._get_icon_overrides(
                tree, "DefaultShortcut.png", group, labelID, False)
    ...
    def _get_overrides(
        ...
        types = {
            "script": os.path.join(
                DEFAULTPATH, "overrides.xml"), "skin": os.path.join(
                SKINPATH, "overrides.xml"), "user": os.path.join(
                profileDir, "overrides.xml")}
        ...
        try:
            ...
            common._save_hash(
                types[overrideType], xbmcvfs.File(
                    types[overrideType]).read())
            ...
            if xbmcvfs.exists(types[overrideType]):
                common.log(
                    "Unable to parse %s overrides.xml. Invalid xml?" %
                    (overrideType))
                common._save_hash(
                    types[overrideType], xbmcvfs.File(
                        types[overrideType]).read())
    ...
    def _get_additionalproperties(self, profileDir):
        ...
        if xbmcvfs.exists(path):
            ...
            try:
                ...
                for listProperty in listProperties:
                    ...
                    # so quickly run it through the local function to remove
                    # the unecessary localisation
                    if listProperty[3].startswith("$SKIN["):
                        listProperty[3] = self.local(listProperty[3])[3]
                    self.currentProperties.append(
        ...
        for elemSearch in [
            [
                "widget", tree.findall("widgetdefault")], [
            ...
            for elem in elemSearch[1]:
                ...
                if elemSearch[0] == "custom":
                    ...
                    if "group" not in elem.attrib:
                        ...
                        self.defaultProperties.append([elem.attrib.get(
                            "group"), labelID, elem.attrib.get('property'), elem.text, defaultID])
                else:
                    # Widget or background
                    if "group" not in elem.attrib:
                        self.defaultProperties.append(["mainmenu", labelID, elemSearch[
                            0].split(":")[0], elem.text, defaultID])
                        ...
                        if elemSearch[0] == "widget":
                            ...
                            widgetDetails = self._getWidgetNameAndType(
                                elem.text)
                        ...
                        self.defaultProperties.append([elem.attrib.get("group"), labelID, elemSearch[
                                                      0].split(":")[0], elem.text, defaultID])
                        ...
                        if elemSearch[0] == "background":
                            ...
                            if backgroundName is not None:
                                self.defaultProperties.append([elem.attrib.get(
                                    "group"), labelID, "backgroundName", backgroundName, defaultID])
                        ...
                        if elemSearch[0] == "widget":
                            ...
                            if widgetDetails is not None:
                                self.defaultProperties.append([elem.attrib.get(
                                    "group"), labelID, "widgetName", widgetDetails["name"], defaultID])
                                if "type" in widgetDetails:
                                    self.defaultProperties.append([elem.attrib.get(
                                        "group"), labelID, "widgetType", widgetDetails["type"], defaultID])
                                if "path" in widgetDetails:
                                    self.defaultProperties.append([elem.attrib.get(
                                        "group"), labelID, "widgetPath", widgetDetails["path"], defaultID])
                                if "target" in widgetDetails:
                                    self.defaultProperties.append([elem.attrib.get(
                                        "group"), labelID, "widgetTarget", widgetDetails["target"], defaultID])

                        if elemSearch[0] == "widget:node":
                            # Set all widget properties from the default
                            if "label" in elem.attrib:
                                self.defaultProperties.append([elem.attrib.get(
                                    "group"), labelID, "widgetName", elem.attrib.get("label"), defaultID])
                            if "type" in elem.attrib:
                                self.defaultProperties.append([elem.attrib.get(
                                    "group"), labelID, "widgetType", elem.attrib.get("type"), defaultID])
                            if "path" in elem.attrib:
                                self.defaultProperties.append([elem.attrib.get(
                                    "group"), labelID, "widgetPath", elem.attrib.get("path"), defaultID])
                            if "target" in elem.attrib:
                                self.defaultProperties.append([elem.attrib.get(
                                    "group"), labelID, "widgetTarget", elem.attrib.get("target"), defaultID])
        ...
        if xbmcvfs.exists(path):
            ...
            for node in tree.getroot().findall("shortcut"):
                ...
                self.defaultProperties.append(
                    ["mainmenu", labelID, "icon", node.find("icon").text])
    ...
    def _getCustomPropertyFallbacks(self, group):
        ...
        for elem in tree.findall("propertyfallback"):
            if ("group" not in elem.attrib and group ==
                    "mainmenu") or elem.attrib.get("group") == group:
                ...
                if propertyName not in fallbackProperties:
                    # Save the property name in the order in which we processed
                    # it
                ...
                # Check whether any attribute/value pair has to match for this
                # fallback
                attribName = None
                attribValue = None
                ...
                if propertyName.startswith(
                        "widgetTarget") and value == "video":
                    value = "videos"
                # Save details
                fallbacks[propertyName].append(
                    (value, attribName, attribValue))
        # Save all the results for this group
        self.propertyInformation["fallbackProperties"][
            group] = fallbackProperties
    ...
    def _getPropertyRequires(self):
        if self.propertyInformation["requires"] is not None:
            # We've already loaded requires and templateOnly properties, return
            # eveything
    ...
    def _reset_backgroundandwidgets(self):
        # This function resets all skin properties used to identify if specific
        # backgrounds or widgets are active
        tree = self._get_overrides("skin")
        for elem in tree.findall("widget"):
            xbmc.executebuiltin(
                "Skin.Reset(skinshortcuts-widget-" + elem.text + ")")
        for elem in tree.findall("background"):
            xbmc.executebuiltin(
                "Skin.Reset(skinshortcuts-background-" + elem.text + ")")

    def createNiceName(self, item, noNonLocalized=False):
        # Translate certain localized strings into non-localized form for
        # labelID
    ...
    def checkAdditionalProperties(
            ...
            isUserShortcuts,
            profileDir):
        # Return any additional properties, including widgets, backgrounds,
        ...
        if isUserShortcuts and (
                len(allProperties[0]) == 0 or allProperties[0][0] is not None):
        ...
        for currentProperty in currentProperties:
            ...
            if labelID is not None and currentProperty[
                    0] == group and currentProperty[1] == labelID:
                returnProperties.append(
                    self.upgradeAdditionalProperties(
                        currentProperty[2], currentProperty[3]))
            elif len(currentProperty) is not 4:
                if defaultID is not None and currentProperty[
                        0] == group and currentProperty[4] == defaultID:
                    returnProperties.append(
                        self.upgradeAdditionalProperties(
                            currentProperty[2], currentProperty[3]))
    ...
    def checkIfMenusShared(self, isSubLevel=False):
        ...
        tree = self._get_overrides("skin")
        if tree is not None:
            # If this is a sublevel, and the skin has asked for sub levels to
            # not be shared...
    ...
    def getSharedSkinList(self):
        ...
        for files in xbmcvfs.listdir(DATAPATH):
            ...
            if files:
                for file in files:
                    if file.endswith(".hash") and not file.startswith(
                            "%s-" % (xbmc.getSkinDir())):
                        canImport, skinName = self.parseHashFile(os.path.join(
                            DATAPATH, file.decode('utf-8')).encode('utf-8'))
                        if canImport:
    ...
    def getFilesForSkin(self, skinName):
        ...
        for files in xbmcvfs.listdir(DATAPATH):
            ...
            if files:
                for file in files:
                    if file.endswith(".DATA.xml") and file.startswith(
                            "%s-" % (skinName)):
                        skinFiles.append(file)
    ...
    def parseHashFile(self, file):
        ...
        for singleHash in hashes:
            if singleHash[0] == "::FULLMENU::":
                canImport = True
                if skinName:
                    return(True, skinName)
            if singleHash[0] == "::SKINDIR::":
                skinName = singleHash[1]
                if canImport:
                    return(True, skinName)
    ...
    def importSkinMenu(self, files, skinName=None):
        ...
        for oldFile in files:
            ...
            oldPath = os.path.join(
                DATAPATH, oldFile.decode('utf-8')).encode('utf-8')
            newPath = os.path.join(
                DATAPATH, newFile.decode('utf-8')).encode('utf-8')
        ...
        propFile = os.path.join(
            DATAPATH, "%s.properties" %
            (xbmc.getSkinDir())).encode('utf-8')
    ...
    def local(self, data):
        ...
        # Get just the integer of the string, for the input forms where this is
        # valid
        ...
        if data.isdigit():
            if int(data) >= 31000 and int(data) < 32000:
                ...
                # If we're on the same skin as the skinid, get the latest
                # translation
                if skinid == xbmc.getSkinDir():
                    lasttranslation = xbmc.getLocalizedString(int(data))
                    returnString = "$SKIN[" + data + "|" + \
                        skinid + "|" + lasttranslation + "]"
                    return [
                        returnString,
                        "$LOCALIZE[" + data + "]",
                        lasttranslation,
                ...
                return [
                    data,
                    "$ADDON[script.skinshortcuts " + data + "]",
                    LANGUAGE(
                        int(data)),
                    data]
    ...
    def slugify(
        ...
        if decimal:
            try:
                text = DECIMAL_REXP.sub(
                    lambda m: unichr(int(m.group(1))), text)
            except:
                pass
        ...
        if hexadecimal:
            try:
                text = HEX_REXP.sub(lambda m: unichr(
                    int(m.group(1), 16)), text)
            except:
                pass
        ...
        # If this is a shortcut file (.DATA.xml) and user shortcuts aren't
        # shared, add the skin dir
        if userShortcuts and self.checkIfMenusShared(isSubLevel) == False:
    ...
    def getListProperty(self, onclick):
        ...
        if onclick.startswith("ActivateWindow"):
            # An ActivateWindow - Let's start by removing the 'ActivateWindow('
            # and the ')'
            listProperty = onclick
            # Handle (the not uncommon) situation where the trailing ')' has
            # been forgotten
            if onclick.endswith(")"):
            ...
            if len(listProperty) == 1:
                ...
                # Situation we haven't anticipated - log the issue and return
                # original onclick
                common.log(
                    "Unable to get 'list' property for shortcut %s" %
                    (onclick))
                return onclick
        else:
    ...
    def upgradeAdditionalProperties(self, propertyName, propertyValue):
        # This function fixes any changes to additional properties between Kodi
        # versions
        if propertyName.startswith(
                "widgetTarget") and propertyValue == "video":
            propertyValue = "videos"

        return [propertyName, propertyValue]

    def buildReplacementMusicAddonAction(self, action, window):
        # Builds a replacement action for an Isengard or earlier shortcut to a
        # specific music addon
