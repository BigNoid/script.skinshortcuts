...
MASTERPATH = os.path.join(
    xbmc.translatePath("special://masterprofile/addon_data/").decode('utf-8'),
    ADDONID).encode('utf-8')
...
class XMLFunctions():
    ...
    def buildMenu(
            ...
            options,
            minitems,
            weEnabledSystemDebug=False,
            weEnabledScriptDebug=False):
        # Entry point for building includes.xml files
        if xbmcgui.Window(10000).getProperty(
                "skinshortcuts-isrunning") == "True":
            return
        ...
        fav_file = xbmc.translatePath(
            'special://userdata/profiles.xml').decode("utf-8")
        tree = None
        ...
        if tree is not None:
            ...
            for profile in profiles:
                ...
                path = profile.find("directory").text.encode("utf-8")
                common.log("Profile found: " + name + " (" + path + ")")
                ...
                if "://" in path:
                    ...
                    path = xbmc.translatePath(
                        os.path.join(
                            ...
                            path)).decode("utf-8")
                profilelist.append([path, "String.IsEqual(System.ProfileName,%s)" % (
                    name.decode("utf-8")), name.decode("utf-8")])
        ...
        if common.attempt(self.writexml,
                          [profilelist,
                           mainmenuID,
                           groups,
                           numLevels,
    ...
    def shouldwerun(self, profilelist):
        try:
            reloadSet = xbmcgui.Window(10000).getProperty(
                "skinshortcuts-reloadmainmenu")
        ...
        xbmc.executebuiltin(
            "Skin.SetString(skinshortcuts-sharedmenu,%s)" %
                ...
                for resolution in resolutions:
                    path = xbmc.translatePath(
                        os.path.join(
                            ...
                            "script-skinshortcuts-includes.xml").encode("utf-8")).decode("utf-8")
                    paths.append(path)
                    ...
                    if xbmc.getCondVisibility(singleHash[1][0]):
                        if singleHash[1][2] == "True":
                            ...
                            xbmc.executebuiltin(
                                "Skin.Reset(%s)" %
                                (singleHash[1][1]))
                elif singleHash[0] == "::FULLMENU::":
                    # Mark that we need to set the fullmenu bool
                    foundFullMenu = True
                elif singleHash[0] == "::SKINDIR::":
                    # Used to import menus from one skin to another, nothing to
                    # check here
                    pass
                else:
                    try:
                        hasher = hashlib.md5()
                        hasher.update(xbmcvfs.File(singleHash[0]).read())
                        if hasher.hexdigest() != singleHash[1]:
                            common.log(
                                "Hash does not match on file " + singleHash[0])
                            common.log(
                                "(" + singleHash[1] + " > " + hasher.hexdigest() + ")")
                            return True
                    except:
                        common.log(
                            "Unable to generate hash for %s" %
                            (singleHash[0]))
                        common.log("(%s > ?)" % (singleHash[1]))
            else:
                if xbmcvfs.exists(singleHash[0]):
                    common.log("File now exists " + singleHash[0])
                    return True
    ...
    def writexml(
        ...
        for checkForShortcutOverride in checkForShorctcutsOverrides:
            if "property" in checkForShortcutOverride.attrib:
                ...
                self.checkForShortcuts.append(
                    (checkForShortcutOverride.text.lower(),
        ...
        for profile in profilelist:
            ...
            for checkforShortcut in self.checkForShortcuts:
                newCheckForShortcuts.append(
                    (checkforShortcut[0], checkforShortcut[1], "False"))
            self.checkForShortcuts = newCheckForShortcuts

            # Clear any previous labelID's
            DATA._clear_labelID()

            # Clear any additional properties, which may be for a different
            # profile
            ...
            # If building the main menu, split the mainmenu shortcut nodes into
            # the menuitems list
            fullMenu = False
            if groups == "" or groups.split("|")[0] == "mainmenu":
                # Set a skinstring that marks that we're providing the whole
                # menu
                xbmc.executebuiltin("Skin.SetBool(SkinShortcuts-FullMenu)")
                hashlist.list.append(["::FULLMENU::", "True"])
                for node in DATA._get_shortcuts(
                        "mainmenu", None, True, profile[0]).findall("shortcut"):
                    menuitems.append(node)
                    submenuItems.append(node)
                fullMenu = True
            else:
                # Clear any skinstring marking that we're providing the whole
                # menu
                xbmc.executebuiltin("Skin.Reset(SkinShortcuts-FullMenu)")
            ...
            for item in menuitems:
                ...
                itemidmainmenu += 1
                currentProgress = (
                    profilePercent * profileCount) + (percent * i)
                ...
                if not isinstance(item, basestring):
                    # This is a main menu item (we know this because it's an
                    # element, not a string)
                    submenu = item.find("labelID").text

                    # Build the menu item
                    menuitem, allProps = self.buildElement(
                        item, "mainmenu", None, profile[1], DATA.slugify(
                            submenu, convertInteger=True), itemid=itemidmainmenu, options=options)
                    ...
                    # Add the menu item to the various includes, retaining a
                    # reference to them
                ...
                for submenuTree in submenuTrees:
                    ...
                    if "noGroups" not in options:
                        if submenu in submenuNodes:
                            ...
                            if count != 0:
                                groupInclude = DATA.slugify(
                                    submenu[:-2], convertInteger=True) + "-" + submenu[-1:]
                            else:
                                groupInclude = DATA.slugify(
                                    submenu, convertInteger=True)

                            justmenuTreeA.set(
                                "name", "skinshortcuts-group-" + groupInclude)
                            justmenuTreeB.set(
                                "name", "skinshortcuts-group-alt-" + groupInclude)

                            submenuNodes[submenu] = [
                                justmenuTreeA, justmenuTreeB]

                    itemidsubmenu = 0

                    # Get the shortcuts for the submenu
                    if count == 0:
                        submenudata = DATA._get_shortcuts(
                            submenu, submenuDefaultID, True, profile[0])
                    else:
                        submenudata = DATA._get_shortcuts(
                            submenu, None, True, profile[0], isSubLevel=True)
                    ...
                    if count == 0:
                        if len(submenuitems) != 0:
                            try:
                                hasSubMenu = xmltree.SubElement(
                                    mainmenuItemA, "property")
                                hasSubMenu.set("name", "hasSubmenu")
                                hasSubMenu.text = "True"
                                if buildMode == "single":
                                    hasSubMenu = xmltree.SubElement(
                                        mainmenuItemB, "property")
                                    hasSubMenu.set("name", "hasSubmenu")
                                    hasSubMenu.text = "True"
                            except:
                    ...
                    # If we're building a single menu, update the onclicks of
                    # the main menu
                    if buildMode == "single" and not len(
                            submenuitems) == 0 and not isinstance(item, basestring):
                        for onclickelement in mainmenuItemB.findall("onclick"):
                            if "condition" in onclickelement.attrib:
                                onclickelement.set(
                                    "condition",
                                    "String.IsEqual(Window(10000).Property(submenuVisibility),%s) + [%s]" %
                                    (DATA.slugify(
                                        submenuVisibilityName,
                                        convertInteger=True),
                                        onclickelement.attrib.get("condition")))
                                newonclick = xmltree.SubElement(
                                    mainmenuItemB, "onclick")
                                newonclick.text = "SetProperty(submenuVisibility," + DATA.slugify(
                                    submenuVisibilityName, convertInteger=True) + ",10000)"
                                newonclick.set(
                                    "condition", onclickelement.attrib.get("condition"))
                            else:
                                onclickelement.set(
                                    "condition",
                    ...
                    for submenuItem in submenuitems:
                        ...
                        menuitem, allProps = self.buildElement(submenuItem, submenu, None, profile[
                                                               1], itemid=itemidsubmenu, mainmenuid=itemidmainmenu, options=options)
                        isSubMenuElement = xmltree.SubElement(
                            menuitem, "property")
                        isSubMenuElement.set("name", "isSubmenu")
                        isSubMenuElement.text = "True"

                        # Save a copy for the template
                        templateSubMenuItems.append(
                            Template.copy_tree(menuitem))
                        ...
                        if "noGroups" not in options:
                            ...
                            visibilityElement.text = "[%s] + %s" % (visibilityElement.text,
                                                                    "String.IsEqual(Window(10000).Property(submenuVisibility),%s)" % (DATA.slugify(
                                                                        submenuVisibilityName,
                                                                        convertInteger=True)))
                            justmenuTreeB.append(menuitemCopy)

                        if buildMode == "single" and not isinstance(
                                item, basestring):
                            # Add the property 'submenuVisibility'
                            allmenuTreeCopy = Template.copy_tree(menuitemCopy)
                            submenuVisibility = xmltree.SubElement(
                                allmenuTreeCopy, "property")
                            submenuVisibility.set("name", "submenuVisibility")
                            submenuVisibility.text = DATA.slugify(
                                submenuVisibilityName, convertInteger=True)
                            allmenuTree.append(allmenuTreeCopy)

                        menuitemCopy = Template.copy_tree(menuitem)
                        visibilityElement = menuitemCopy.find("visible")
                        visibilityElement.text = "[%s] + %s" % (visibilityElement.text,
                                                                "String.IsEqual(Container(%s).ListItem.Property(submenuVisibility),%s)" % (mainmenuID,
                                                                                                                                           DATA.slugify(
                                                                                                                                               submenuVisibilityName,
                                                                                                                                               convertInteger=True)))
                    ...
                    Template.parseItems(
                        ...
                        None,
                        buildOthers,
                        mainmenuitems=templateCurrentMainMenuItem)
            ...
            if not self.hasSettings:
                ...
                if forceSettings is not None:
                    ...
                    xmltree.SubElement(
                        newelement, "label").text = "$LOCALIZE[10004]"
                    xmltree.SubElement(
                        newelement, "icon").text = "DefaultShortcut.png"
                    xmltree.SubElement(
                    ...
                    if buildMode == "single":
                        ...
                        xmltree.SubElement(
                            newelement, "onclick").text = "ActivateWindow(settings)"
                        xmltree.SubElement(
                            newelement, "visible").text = profile[1]

            if len(self.checkForShortcuts) != 0:
                # Add a value to the variable for all checkForShortcuts
                for checkForShortcut in self.checkForShortcuts:
                    if profile[1] is not None and xbmc.getCondVisibility(profile[
                                                                         1]):
                        # Current profile - set the skin bool
                        if checkForShortcut[2] == "True":
                            xbmc.executebuiltin(
                                "Skin.SetBool(%s)" % (checkForShortcut[1]))
                        else:
                            xbmc.executebuiltin(
                                "Skin.Reset(%s)" % (checkForShortcut[1]))
                    # Save this to the hashes file, so we can set it on profile
                    # changes
                    hashlist.list.append(
                        ["::SKINBOOL::", [profile[1], checkForShortcut[1], checkForShortcut[2]]])

            # Build the template for the main menu
            Template.parseItems(
                "mainmenu",
                0,
                templateMainMenuItems,
                profile[2],
                profile[1],
                "",
                "",
            ...
            while itemidmainmenu < minitems and fullMenu and len(
                    mainmenuTree) != 0:
    ...
    def buildElement(
        ...
        xmltree.SubElement(
            newelement,
            "label").text = DATA.local(
            item.find("label").text)[1]
        xmltree.SubElement(
            newelement,
            "label2").text = DATA.local(
            item.find("label2").text)[1]
        ...
        if icon is None:
            ...
            xmltree.SubElement(
                newelement,
                "icon").text = common.try_decode(
                icon.text)
        thumb = item.find("thumb")
        if thumb is not None:
            xmltree.SubElement(
                newelement, "thumb").text = common.try_decode(
                item.find("thumb").text)
        ...
        if item.find("disabled") is not None:
            # It is, so we set it to be invisible, add an empty onclick and
            # return
        ...
        if len(properties) != 0:
            for singleProperty in properties:
                if singleProperty[0] == "node.visible":
                    visibleProperty = xmltree.SubElement(newelement, "visible")
                    visibleProperty.text = common.try_decode(singleProperty[1])
                else:
                    additionalproperty = xmltree.SubElement(
                        newelement, "property")
                    additionalproperty.set("name", singleProperty[0].decode("utf-8"))
                    additionalproperty.text = singleProperty[1]
                    allProps[singleProperty[0]] = additionalproperty

                    # If this is a widget or background, set a skin setting to
                    # say it's enabled
                    if singleProperty[0] == "widget":
                        xbmc.executebuiltin(
                            "Skin.SetBool(skinshortcuts-widget-" + singleProperty[1] + ")")
                        # And if it's the main menu, list it
                        if groupName == "mainmenu":
                            xbmc.executebuiltin(
                                "Skin.SetString(skinshortcuts-widget-" + str(self.widgetCount) + "," + singleProperty[1] + ")")
                            self.widgetCount += 1
                    elif singleProperty[0] == "background":
                        try:
                            xbmc.executebuiltin(
                                "Skin.SetBool(skinshortcuts-background-" + singleProperty[1] + ")")
                        except UnicodeEncodeError:
                            xbmc.executebuiltin(
                                "Skin.SetBool(skinshortcuts-background-" + singleProperty[1].encode('utf-8') + ")")
                    ...
                    if groupName == "mainmenu":
                        if "clonewidgets" in options:
                            widgetProperties = [
                                ...
                                "widgetPath",
                                "widgetPlaylist"]
                            if singleProperty[0] in widgetProperties:
                                self.MAINWIDGET[singleProperty[0]] = singleProperty[1]
                        if "clonebackgrounds" in options:
                            ...
                            if singleProperty[0] in backgroundProperties:
                                self.MAINBACKGROUND[singleProperty[0]] = singleProperty[1]
                        if "cloneproperties" in options:
                            self.MAINPROPERTIES[singleProperty[0]] = singleProperty[1]

                    # For backwards compatibility, save widgetPlaylist as
                    # widgetPath too
                    if singleProperty[0] == "widgetPlaylist":
                        additionalproperty = xmltree.SubElement(
                            newelement, "property")
                        additionalproperty.set("name", "widgetPath")
                        additionalproperty.text = common.try_decode(property[
                                                                    1])

        # Get fallback properties, property requirements, templateOnly value of
        # properties
                ...
                for propertyMatch in fallbacks[key]:
                    ...
                    if propertyMatch[1] is None:
                        ...
                        for property in properties:
                            if property[0] == propertyMatch[
                                    1] and property[1] == propertyMatch[2]:
                                matches = True
                                break
        ...
        for onclick in onclicks:
            ...
            if "condition" in onclick.attrib:
                onclickelement.set(
                    "condition", onclick.attrib.get("condition"))
        ...
        for onclick in onclicks:
            ...
            if onclick.text.startswith("pvr-channel://"):
                ...
                try:
                    ...
                    actionParts[1] = actionParts[1].replace(
                    ...
                    if len(actionParts) == 2:
                        onclickelement.text = "ActivateWindow(" + actionParts[
                            0] + "," + newAction + ")"
                    else:
                        onclickelement.text = "ActivateWindow(" + actionParts[
                            0] + "," + newAction + "," + actionParts[2] + ")"
                except:
                    pass
            else:
                onclickelement.text = onclick.text
            ...
            if not self.propertyExists(
                    "path", newelement) and not "path" in allProps.keys():
                # we only add the path property if there isn't already one in
                # the list because it has to be unique in Kodi lists
            ...
            # Get 'list' property (the action property of an ActivateWindow
            # shortcut)
            if not self.propertyExists(
                    "list", newelement) and not "list" in allProps.keys():
                # we only add the list property if there isn't already one in
                # the list because it has to be unique in Kodi lists
                listElement = xmltree.SubElement(newelement, "property")
                listElement.set("name", "list")
                listElement.text = DATA.getListProperty(
                    onclickelement.text.replace('"', ''))
            ...
            if len(self.checkForShortcuts) != 0:
                ...
                for checkforShortcut in self.checkForShortcuts:
                    if onclick.text.lower() == checkforShortcut[0]:
                        ...
                        newCheckForShortcuts.append(
                            (checkforShortcut[0], checkforShortcut[1], "True"))
                    else:
        ...
        if submenuVisibility is not None:
            submenuVisibilityElement = xmltree.SubElement(
                newelement, "property")
            submenuVisibilityElement.set("name", "submenuVisibility")
            if submenuVisibility.isdigit():
                submenuVisibilityElement.text = "$NUMBER[" + \
                    submenuVisibility + "]"
            else:
        ...
        # If this isn't the main menu, and we're cloning widgets or
        # backgrounds...
        if groupName != "mainmenu":
            if "clonewidgets" in options and len(self.MAINWIDGET) is not 0:
                for key in self.MAINWIDGET:
                    ...
                    additionalproperty.text = common.try_decode(
                        self.MAINWIDGET[key])
                    allProps[key] = additionalproperty
            if "clonebackgrounds" in options and len(
                    self.MAINBACKGROUND) is not 0:
                for key in self.MAINBACKGROUND:
                    additionalproperty = xmltree.SubElement(
                        newelement, "property")
                    additionalproperty.set("name", key)
                    additionalproperty.text = DATA.local(
                        self.MAINBACKGROUND[key])[1]
                    allProps[key] = additionalproperty
            if "cloneproperties" in options and len(
                    self.MAINPROPERTIES) is not 0:
                for key in self.MAINPROPERTIES:
                    additionalproperty = xmltree.SubElement(
                        newelement, "property")
                    additionalproperty.set("name", key)
                    additionalproperty.text = DATA.local(
                        self.MAINPROPERTIES[key])[1]
                    allProps[key] = additionalproperty
            ...
            for propertyName in propertyPatterns:
                ...
                for original, replacement in propertyReplacements:
                    regexpPattern = re.compile(
                        re.escape(original), re.IGNORECASE)
                    propertyPattern = regexpPattern.sub(
                        replacement, propertyPattern)
        ...
        for propertyPatternElement in self.propertyPatterns:
            ...
            if not propertyLabelID:
                if propertyName not in propertyPatterns:
                    propertyPatterns[propertyName] = [
                        propertyPatternElement.text, False]
            elif propertyLabelID == labelID:
                if propertyName not in propertyPatterns or propertyPatterns[
                        propertyName][1] == False:
                    propertyPatterns[propertyName] = [
                        propertyPatternElement.text, True]
        ...
        for subElement in list(element):
            if subElement.tag == "property":
                ...
                if propertyName and subElement.text:
                    propertyReplacements.append(
                        ("::%s::" % propertyName, subElement.text))
            elif subElement.text:
                propertyReplacements.append(
                    ("::%s::" % subElement.tag, subElement.text))
