...
class Template():
    ...
    def __init__(self):
        ...
        try:
            ...
            if xbmcvfs.exists(templatepath):
                ...
                common._save_hash(
                    templatepath, xbmcvfs.File(templatepath).read())
            else:
                # No template.xml
                self.tree = None
                common._save_hash(templatepath, None)

        # Empty variable which will contain our base elementree (passed from
        # buildxml)
        self.includes = None

        # Empty progress which will contain the Kodi progress dialog gui
        # (passed from buildxml)
        self.progress = None
        self.percent = None
        self.current = None
    ...
    def parseItems(
        ...
        if template is not None:
            ...
            self.getInclude(
                self.includes, includeName, profileVisibility, profile)
            includeTree = self.getInclude(
                self.includes, includeName + "-%s" %
                (profile), None, None)

            # If we've been passed any mainmenu items, retrieve their
            # properties
            properties = {}
        ...
        # If we've been passed any mainmenu items, retrieve the id of the main
        # menu item
        rootID = None
        if mainmenuitems is not None:
        ...
        if menuType == "mainmenu":
            ...
            # Switch menutype for level for submenu templates, to make it
            # easier to pass in
            menuType = level
        for item in items:
            ...
            if menuType == "mainmenu":
                ...
                for element in item.findall("property"):
                    if "name" in element.attrib and element.attrib.get(
                            "name") == "submenuVisibility":
                        visibilityName = element.text
                        break

                finalVisibility = "String.IsEqual(Container(%s).ListItem.Property(submenuVisibility),%s)" % (
                ...
                finalVisibility = "[%s + String.IsEqual(Container(::SUBMENUCONTAINER::).ListItem.Property(labelID),%s)]" % (
            ...
            numTemplates += self.findOther(item,
                                           profile,
                                           profileVisibility,
                                           visibilityCondition,
                                           finalVisibility,
    ...
    def writeOthers(self):
        ...
        for template in self.finalize:
            ...
            if "include" in template.attrib:
                ...
                name += "-%s" % (includeName)
                # Remove the include from our list of other templates, as we
                # don't need to build an empty one
            ...
            for profile in template.findall("skinshortcuts-profile"):
                ...
                include = self.getInclude(
                    ...
                    (name,
                     profile.attrib.get("profile")),
                    None,
                    None)  # profile.attrib.get( "visible" ) )

                # Create a copy of the node with any changes within (this time
                # it'll be visibility)
                ...
                if variables is not None:
                    for variable in variables.findall("variable"):
                        # If the profile doesn't have a dict in finalVariables,
                        # create one
                        ...
                        if varName in finalVariables[profileVisibility].keys():
                            newVariables = finalVariables[
                                profileVisibility][varName]
                        ...
                        for value in variable.findall("value"):
                            ...
                            # Add the new condition/value pair only if it
                            # really is new
        ...
        for variableName in finalVariableNames:
            ...
            for condition, value in self.parseVariables(
                    variableName, finalVariables):
                valueElement = xmltree.SubElement(element, "value")
                valueElement.text = value
                if condition != "":
                    valueElement.set("condition", condition)

        # If there are any 'other' templates that we haven't built, build an
        # empty one
        for otherTemplate in self.otherTemplates:
            ...
            xmltree.SubElement(
                root,
                "description").text = "This include was built automatically as the template didn't match any menu items"

    def parseVariables(self, variableName, allVariables):
        # This function will return all condition/value elements for a given
        # variable, including adding profile conditions
        returnVariables = []
        noCondition = []

        # Firstly, lets pull out the specific variables from all the variables
        # we've been passed
        limitedVariables = {}
            ...
            while len(limitedVariables[profile]) != 0:
                ...
                for additionalProfile in limitedVariables:
                    if value in limitedVariables[additionalProfile]:
                        # It does - remove it and add the profile visibility to
        ...
        for elem in self.tree.findall(searchType):
            ...
            if menuType != "mainmenu":
                ...
                if "container" in elem.attrib:
                    finalVisibility = visibilityCondition.replace(
                        "::SUBMENUCONTAINER::", elem.attrib.get("container"))
                else:
                    finalVisibility = simpleVisibility

            # Check whether the skinner has set the match type (whether all
            # conditions need to match, or any)
            matchType = "all"
            ...
            for condition in template.findall("condition"):
                if matchType == "all":
                    ...
                    if matched:
                        break
                    if self.checkCondition(condition, item):
                        matched = True
                        break
            ...
            self.replaceElements(
                ...
                properties)
            self.replaceElements(
                template.find("variables"),
                None,
                None,
                [],
                properties)

            # Now we need to check if we've already got a template identical to
            # this
            ...
            for previous in self.finalize:
                ...
                if previous.find(
                        "skinshortcuts-includeName").text != includeNameCheck:
                    continue

                # Compare templates
                if self.compare_tree(
                        template.find("controls"),
                        previous.find("controls")) and self.compare_tree(
                        template.find("variables"),
                        previous.find("variables")):
                    # They are the same

                    # Add our details to the previous version, so we can build it
                    # with full visibility details later
                    for profileMatch in previous.findall(
                            "skinshortcuts-profile"):
                        if profileMatch.attrib.get("profile") == profile:
                            ...
                            xmltree.SubElement(
                                profileMatch, "visible").text = finalVisibility
                            foundInPrevious = True
            ...
            if not foundInPrevious:
                # We don't have this template saved, so add our profile details
                # to it
                newElement = xmltree.SubElement(
                    template, "skinshortcuts-profile")
                newElement.set("profile", profile)
                newElement.set("visible", profileVisibility)
                ...
                newElement = xmltree.SubElement(
                    template, "skinshortcuts-includeName")
                if includeName is None:
    ...
    def getProperties(self, elem, items):
        ...
        for singleProperty in searchProperties:
            if "name" not in singleProperty.attrib or singleProperty.attrib.get(
                    "name") in properties:
                # Name attrib required, or we've already got a property with
                # this name
                continue
            name = singleProperty.attrib.get("name")

            # Pull out the tag, attribute and value attribs into an array of tuples
            rules = []
            matchAny = True
            propertyValue = None
            if "propertyValue" in singleProperty.attrib:
                propertyValue = singleProperty.attrib.get("propertyValue")

            # Check for multiple items to match against this single value
            for singleMatch in singleProperty.findall("rule"):
                tag = None
                attribute = None
                value = None
                if "tag" in singleMatch.attrib:
                    ...
                    common.log(
                        "Trying to match a property without using a tag element")
                    continue
            ...
            if len(rules) == 0:
                if "tag" in singleProperty.attrib:
                    ...
                    value = None
                    propertyValue = None
                    if "attribute" in singleProperty.attrib:
                        attribute = singleProperty.attrib.get("attribute").split("|")
                    if "value" in singleProperty.attrib:
                        value = singleProperty.attrib.get("value").split("|")
                    if singleProperty.text:
                        propertyValue = singleProperty.text
                    rules.append((tag, attribute, value, propertyValue))
                else:
                    # No tag property, so this will always match (so let's just
                    # use it!)
                    if singleProperty.text:
                        properties[name] = singleProperty.text
                    else:
                        properties[name] = ""
                    continue
            ...
            if matchAny:
                    ...
                    for item in items.findall(tag):
                        ...
                        if attrib is not None:
                            if attrib[0] not in item.attrib:
                                ...
                                continue
                            if attrib[1] != item.attrib.get(attrib[0]):
                                # The attributes value doesn't match, so we
                                # don't want to check the rule against it
                                continue
                ...
                if matchedRule:
                    ...
                    if rule[3] is not None:
                        ...
                        # include a log error
                        common.log(
                            "Invalid template - cannot set property directly to menu item elements value when using multiple rules for single property")
        ...
        for elem in tree:
            ...
            if "skinshortcuts" in elem.attrib:
                ...
                for singleAttrib in elem.attrib:
                    if singleAttrib == "skinshortcuts":
                        replaceType = elem.attrib.get("skinshortcuts")
                    else:
                        attribs.append(
                            (singleAttrib, elem.attrib.get(singleAttrib)))
                text = elem.text
                tag = elem.tag

                # Don't continue if type = visibility, and no
                # visibilityCondition
                if replaceType == "visibility" and visibilityCondition is None:
                    continue
                ...
                # Make replacements
                if replaceType == "visibility" and visibilityCondition is not None:
            ...
            if elem.text is not None:
                while "$SKINSHORTCUTS[" in elem.text:
                    ...
                    # stringEnd[ 1 ] = Any code after the $SKINSHORTCUTS
                    # property
                    ...
                    if stringEnd[0] in properties:
                        if properties[stringEnd[0]].startswith("$INCLUDE["):
                            ...
                            elem.text = ""
                            # Add include element
                            includeElement = xmltree.SubElement(
                                elem, "include")
                            includeElement.text = properties[
                                stringEnd[0]][9:-1]
                        else:
                            elem.text = stringStart[
                                0] + properties[stringEnd[0]] + stringEnd[1]
                    else:
            ...
            for attrib in elem.attrib:
                ...
                while "$SKINSHORTCUTS[" in elem.attrib.get(attrib):
                    # Split the string into its composite parts
                    stringStart = elem.attrib.get(
                        attrib).split("$SKINSHORTCUTS[", 1)
                    stringEnd = stringStart[1].split("]", 1)
                ...
                if value.startswith("$SKINSHORTCUTS[") and value[
                        15:-1] in properties:
                    newValue = ""
            ...
            if elem.text is not None:
                while "$PYTHON[" in elem.text:
                    ...
                    stringEnd[0] = simple_eval(
                        "%s" %
                        (stringEnd[0]),
                        names=properties)

                    elem.text = stringStart[0] + \
                        str(stringEnd[0]) + stringEnd[1]
            ...
            for attrib in elem.attrib:
                ...
                while "$PYTHON[" in elem.attrib.get(attrib):
                    ...
                    elem.set(attrib, stringStart[
                             0] + str(stringEnd[0]) + stringEnd[1])
            ...
            if elem.tag == "skinshortcuts":
                ...
                # Don't continue is type = visibility, and no
                # visibilityCondition
                ...
                if type == "visibility" and visibilityCondition is not None:
                    ...
                    for elem in self.buildSubmenuCustomItems(
                            customitems, items.findall("item"), elem.attrib.get("insert"), properties):
                        for child in elem:
                            tree.insert(index, child)
                elif type == "items":
                    # Firstly, go through and create an array of all items in reverse order, without
                    # their existing visible element, if it matches our
                    # visibilityCondition
                    newelements = []
                    if items == []:
                        break
    ...
    def buildSubmenuCustomItems(
        ...
        for item in items:
            ...
            self.replaceElements(
                newElement, None, None, [], self.combineProperties(
                    itemTemplate, item, currentProperties.copy()))
